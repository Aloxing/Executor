...
async function submit() {
  ...
  saving.value = true
  try {
    const queue = await createConfigQueue({
      name: name.value,
      uuid: uuid.value,
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-[3%]">
    ...
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="create-config-queue-title"
